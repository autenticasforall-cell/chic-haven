    ...
    (set, get) => ({
      ...
      addItem: async (item) => {
        ...
        try {
          ...
          if (import.meta.env.DEV) console.error('Failed to add item:', error);
        } finally {
          set({ isLoading: false });
        }
      },
      ...
      applyDiscount: async (code: string) => {
        ...
        try {
          ...
          const applied = discountCodes.some((d: { applicable: boolean }) => d.applicable);
          if (applied) set({ discountCode: code });
          return applied;
        } catch {
          return false;
        }
      ...
      syncCart: async () => {
        ...
        try {
          ...
          if (import.meta.env.DEV) console.error('Failed to sync cart:', error);
        } finally {
          set({ isSyncing: false });
        }
      },
